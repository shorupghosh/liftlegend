...
export default function UsageAnalytics() {
        ...
        try {
            ...
            for (let i = 6; i >= 0; i--) {
                ...
                const { count } = await supabase.from('attendance').select('id', { count: 'exact', head: true })
                    .gte('check_in_time', d.toISOString()).lt('check_in_time', next.toISOString());
    ...
    const maxCount = Math.max(...weeklyCheckins.map(d => d.count), 1);
    const maxMembers = Math.max(...topGyms.map(g => g.memberCount), 1);
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    ...
                    <div key={s.label} className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                        ...
                        <p className="text-2xl font-black text-neutral-text dark:text-white">{loading ? '—' : s.value.toLocaleString()}</p>
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                    ...
                    {loading ? (
                        ...
                        <div className="p-12 text-center text-slate-500">No gym data available.</div>
                    ) : (
